...
Rig uses package-name `effect` for the v4 beta:
...
Import code from package-name `effect`:
...
- Rig tests provide those with `FileSystem.layerNoop({})`, `Path.layer`,
  `Terminal.make(...)`, `Stdio.layerTest(...)`, and a no-op
  `ChildProcessSpawner`.
