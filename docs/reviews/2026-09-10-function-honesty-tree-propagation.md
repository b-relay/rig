...
| 6 | `createProcessInspection` `process-inspection.ts:32-35` defaults `run`/`kill` | → `createChildSupervisor` `[D+T]` → owners | yes, require both; its members are already honest given them. (fixed by #225: both required, `platformKill` names the platform path) |
| 7 | `createChildSupervisor` `child-supervisor.ts:66-67` defaults `now`, `inspection`; `composeDaemon:42` passes neither | → `composeDaemon` `[O+T]` → `rigd main`; → `runCapturedProcess` `[O+T]` | own defect clears by requiring the options (or passing them at `composition.ts:42`); stays `[T]` until 2-5 clear. (fixed by #225: `timing` and `processInspection` required; both owners pass them) |
...
| 19 | `doctor` `runtime/doctor.ts:155` same omission (plus #119/#126 result-shape defects; #119 fixed: one `acquireDocument` per report, `config-unreadable` distinct from `config-invalid`) | → `execute` `[D+T]` → `command` `[T]` → `createRuntime` → owners | yes with the same timing fix. |
...
| config validation gaps (union "Invalid input", spurious `base.` override error, interpolation without path, Caddy-invalid domains, `ports` namespace collision, `.bak` litter) | #164 (fixed: union explains the missing kind, override check only on real overrides, interpolation errors carry the field path and a hint, `domain`/`subdomain` must be hostnames at parse and after interpolation, `port`/`ports` names refused, one `rig.yaml.bak` replaced per edit) |
...
| minor logs gaps (UTC times, cursor error on unreadable file, wrapper logs invisible, no Target log rotation, build output burst) | #169 (fixed: `Z` marker on log times, `LOG_UNREADABLE` names the file, launchd `<component>.stdout/stderr.log` shown, `target.jsonl` rotates at 64 MiB with one previous generation and a rotation-aware follow, hook/build output streamed line by line through `CommandRequest.onOutput`, wrapper republishes unchanged evidence every 250 ms instead of 50 ms) |
...
| health minors (3xx unhealthy without reason, evidence discarded, dependsOn readiness weaker than documented, readyTimeout overflow) | #176 (fixed: `health` returns a `HealthCheck` with the failure reason, redirects count as ready, evidence changes are logged as `health` lines, `HEALTH_FAILED` carries `lastCheck`, a running dependency is re-checked before a dependent starts, timeouts capped at 86400 s) |
...
| `init --path` registers the Git toplevel, ignores nearer `rig.yaml` | #178 | (fixed: `inspectInitialization` registers the nearest config at or above the path inside the repository (`nearestConfigWithin`), the same document `discoverProject` finds; only a repository without one scaffolds at the Git root)
| non-interactive `init` records the checked-out branch as Production | #179 (fixed: `inspectProjectLocation` reports `origin/HEAD` and the checkout separately; `inspectInitialization` prefers `--production-branch`, `origin/HEAD`, then the host default) |
| registration minors (same-name second repo, init discards flags, rename to same name, repoint non-git dir, no unregister, discovery above nested root) | #180 (fixed: `PROJECT_CONFLICT` names the config or `--project` as the way to another name; `init` over an existing config warns which flags were not applied; `rename <current>` is "unchanged" without a rewrite; `discover` reports `gitRequired` and `repoint` refuses a non-repository with `GIT_REQUIRED`; `rig forget <name>` removes a stopped registration and `list` marks a missing directory; the adapter's `discover` stops at the nearest Git toplevel) |
...
| doctor discards observation reason and exit code for failing components | #200 (fixed: `componentCheck` in doctor.ts carries reason/exitCode into the message and picks the hint from them) |
...
| doctor/status/list minors (aggregate never unhealthy, project note dead code, list observes for nothing, deployBranch not drift, caddy-reload check unreachable, no commit) | #203 (fixed: `aggregate` counts every Component and returns `unhealthy`; doctor reports return `project` and a `note` saying why Project checks were skipped, including an unregistered Project; `list` no longer observes Targets; `live/branch` check reports `production-branch-drift`; dead `caddy-reload` check removed; status shows `branch@commit`) |
| client deadline expiry reported as DAEMON_UNREACHABLE while rigd keeps executing; retry queues a duplicate | #204 |
| Caddy validate/reload stderr discarded on every path, rejected file deleted; missing caddy generic COMMAND_START | #205 | (fixed: `ROUTE_VALIDATE`/`ROUTE_RELOAD` carry the last stderr line in the message and `details.evidence`; the rejected text is kept at `<file>.rejected` and named in the hint; a `COMMAND_START` from caddy becomes `CADDY_UNAVAILABLE`; `runCommand` names the executable and cause; `inspectHost` adds `provider/caddy`; `diagnosticEvidence` copies a failure's opted-in `details.evidence` into the diagnostic record, bounded to 500 printable characters)
| Caddy/daemon minors (port-unaware conflict, remove() no-op reload, capture raw stack, Origin check not a rebinding defence) | #206 (fixed: `siteAddress` compares host and default port; `change` returns before writing when nothing is owned and nothing is added; `rigd capture` prints message and hint, usage on bad arity; Origin compared with the daemon's own loopback origins, never the Host header) |
