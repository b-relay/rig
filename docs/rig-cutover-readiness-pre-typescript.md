> Historical record of the pre-September 9 Effect rewrite. This is not current
> architecture, supported configuration, Host state, or operational guidance.
> See [current cutover readiness](rig-cutover-readiness.md). Machine-specific
> commands and prior validation claims below must not be reused as current proof.
