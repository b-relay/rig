...
| Lifecycle start | `rig start <name> dev|prod` | `rig2 up --lane local|live` | partial: routes through `rigd` receipts/log state; provider-backed runtime execution pending | blocked by #25 |
| Lifecycle stop | `rig stop <name> dev|prod` | `rig2 down --lane local|live` | partial: routes through `rigd`; provider-backed runtime execution pending | blocked by #25 |
...
| Status | `rig status` | `rig2 status` | partial: reports foundation and `rigd` state; runtime execution parity pending | blocked by #25 |
| Logs | `rig logs` | `rig2 logs` | partial: reads structured `rigd` logs; provider-backed component log ingestion pending | blocked by #25 |
| Deploy | `rig deploy <name> dev|prod` | `rig2 deploy --target live|generated --ref <ref>` | partial: deploy intent and generated state exist; provider-backed deploy pending | blocked by #25 |
