...
| Lifecycle start | `rig start <name> dev|prod` | `rig2 up --lane local|live` | partial: repo-inferred and `--config` paths load validated v2 config and route config-backed `rigd` lifecycle writes through ordered runtime provider methods; `native-health` now performs HTTP checks; broader first-party adapter parity still pending | #25 in progress |
