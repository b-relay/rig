...
> Status: created in GitHub as #55 through #62.
...
- [ ] `rig bump` is removed from normal CLI behavior and help.
- [ ] Normal `rig` help does not expose `--state-root`, generic `--config`,
  ...
  provider choices.
- [ ] Removed or hidden surfaces have public-behavior tests.
- [ ] Remaining help text uses Project, Target, Branch, Commit, Stable Target,
...
- [ ] `rigd install` sets up daemon state and creates the local control-plane
  auth token.
- [ ] `rigd status` reports daemon installation, running, and reachability
  state without becoming a project inventory command.
- [ ] `rigd uninstall` removes daemon admin artifacts but refuses while
  Rig-managed Targets are running.
- [ ] Normal `rig` commands do not install or manually start `rigd`; they fail
...
- [ ] `rig init` resolves the Git repository root from any subdirectory and
...
- [ ] Interactive init confirms Project identity and Production branch;
  ...
  provided.
- [ ] `rig init` configures the `rig` Git remote when absent, continues when it
  already points to the expected Rig remote, and refuses to overwrite a
  different remote.
- [ ] Full init success requires registration with `rigd`.
- [ ] Partial config-written registration failure is reported clearly and can be
  completed by rerunning `rig init`.
- [ ] Duplicate Project identity/path conflicts are rejected.
...
- [ ] `rig list` reads daemon-backed Host inventory and shows Project summaries
  with Target counts.
- [ ] `rig list` fails when `rigd` is unreachable instead of reading stale
  files.
- [ ] `rig status` is Project-scoped and shows all Targets for one selected
  Project.
- [ ] `rig status` fails outside a Project unless `--project <name>` resolves to
  a Project known to `rigd`.
- [ ] `rig doctor` runs Host diagnostics even outside a Project, adds Project
  diagnostics when context exists, and remains read-only by default.
- [ ] `rig doctor` reports Project identity drift, duplicate identity/path
...
- [ ] `rig deploy preview` deploys the current Branch and fails from detached
  HEAD.
- [ ] `rig deploy preview <branch>` deploys an explicit local Branch.
- [ ] Preview deploys from the Production branch itself are rejected with
  guidance to create a Preview branch such as `preview/main`.
- [ ] Same-Commit deploys are no-ops unless `--force` is used and do not start a
  stopped Target.
- [ ] `--no-up` materializes without starting; replacing a running Target with
...
- [ ] Pushing the Production branch to the Rig remote updates the Stable Target
  and brings it up by default.
- [ ] Pushing any non-Production destination Branch updates a Preview and brings
  it up by default.
- [ ] `git push rig main:preview/main` classifies by destination Branch and
  deploys a Preview.
- [ ] Same-Commit pushes are no-ops and do not start stopped Targets.
- [ ] Rig remote pushes do not run local upstream/ahead/behind preflight checks.
...
- [ ] `rig up`, `rig down`, `rig restart`, and `rig logs` share Target selection:
  interactive picker in TTY, fail with guidance in non-interactive use.
- [ ] Bare Target names select Working copy or Stable Targets; Previews require
  `preview <branch>`.
- [ ] Missing Preview lifecycle commands fail with guidance to deploy first.
- [ ] `down` stops a Target without removing it from inventory.
- [ ] `restart` may start an existing stopped Target but does not redeploy or
  change Commit.
- [ ] `logs` prints recent combined stdout/stderr and exits by default.
- [ ] `logs --follow` streams.
- [ ] `logs` can read stopped Target logs when logs exist.
...
- [ ] `rigd` resolves Project config and Host config into a runtime plan before
  calling providers.
- [ ] Provider calls receive shared Runtime context plus typed
  provider-specific config.
- [ ] Caddy-specific paths and route config are passed only to the Caddy provider.
- [ ] Providers do not read home config, Project config, or global path helpers
  directly.
- [ ] Tests prove providers can run against injected context.
- [ ] First-party provider contracts leave room for future third-party providers
