# Plan: rig runtime authority deepening
...
- **Runtime plan**: rig config resolution should produce a rig runtime plan using
...
## Phase 26: Introduce Rig Runtime Plan For One Lifecycle Path
...
Make rig config resolution produce a first-class runtime plan for one local/live
lifecycle path. The runtime plan should use Rig concepts directly:
...
- [x] Config resolution returns a rig runtime plan for at least one lifecycle
      path.
...
Move deploy execution and preflight evidence toward rig runtime-plan inputs. The
...
- [x] Deploy execution can consume the rig runtime plan for at least one
      complete path.
- [x] Preflight can consume the rig runtime plan for at least one complete check
      path.
...
Preserve rig label isolation, plist rendering, launchctl bootstrap/bootout
...
- [x] Launchd process supervisor implementation lives in a focused adapter
      module.
- [x] Plist rendering preserves rig namespace and label isolation.
...
rig-managed block parsing, v1 marker compatibility where still needed during
...
- [x] Rig-managed block parsing and rendering remain externally compatible.
