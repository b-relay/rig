...
- **Cutover safety**: the rig implementation remains isolated from legacy runtime state. The final cutover is a deliberate removal of v1 code followed by making rig the `rig` CLI.

---

## Phase 14: Define Rig Provider Plugin Contracts
...
Deepen the rig provider model into explicit Effect v4 service interfaces for process supervision, proxy/routing, SCM, workspace/deployment materialization, logging/event transport, control-plane transport, health checking, package-manager integration, and optional tunnel/exposure providers.

### Acceptance Criteria

- [x] Rig provider families are represented as explicit interfaces/services, not concrete provider imports in core logic.
...
- [x] Tests prove rig runtime code can swap provider compositions without changing command or core modules.
- [x] Existing v1 provider behavior remains compatible during migration.

### Current Output

`src/rig/provider-contracts.ts` defines explicit Effect service tags for each
provider family plus the rig provider registry service and shared plugin
...
Make `rigd` persist deployment inventory snapshots, action receipts, structured runtime events, health summaries, port reservations, provider reconciliation state, and bounded recovery evidence under the isolated rig state root.

### Acceptance Criteria

- [x] `rigd` persists action receipts, runtime events, deployment state, and health summaries under the rig state root.
...
`src/rig/rigd-state.ts` defines the `RigdStateStore` interface with file and
...
`src/rig/control-plane.ts` defines the localhost-first control-plane service,
...
`src/rig/rigd-actions.ts` defines an injectable action preflight interface so
...
Add the config edit workflow required by the future web control plane: read current rig config, propose a structured patch, validate it through Effect Schema, preview the diff, apply it atomically, and report rollback/recovery information.

The target product behavior is that all rig config fields can be edited through
...
- [x] All rig config fields are representable by the edit model, even if advanced UI controls land incrementally.
- [x] Proposed config edits are represented as structured patches rather than arbitrary string writes.
- [x] Every edit is validated with rig Effect Schema before writing.
...
`src/rig/config-editor.ts` defines the config editor interfaces for reading,
previewing, and applying rig config edits through structured patch operations.
...
The patch model uses path arrays and `set`/`remove` operations, so every rig
...
## Phase 20: Prepare Rig To Main Rig Cutover Readiness
...
isolated rig implementation when it is safe. The long-term goal is that rig
becomes `rig`; the temporary rig entrypoint is migration runway, not a permanent
second product.

There are no external users to preserve compatibility for. The cutover is a
deliberate v1 removal and rig promotion, not gradual command routing through v1.
The safety requirement is to keep current machine state isolated until the
replacement is deliberate.

### Acceptance Criteria

- [x] A cutover readiness checklist exists in docs and maps final CLI areas to their rig readiness status.
...
- [x] The plan explicitly covers deleting v1 and promoting rig behavior as `rig` when rig becomes default.
- [x] Remaining post-cutover gaps are filed as follow-up issues instead of hidden in the plan.

### Current AFK Output

`docs/rig-cutover-readiness.md` records the replacement readiness matrix,
provider safety requirements, validation checklist, rollback checklist, completed
promotion, and remaining gaps now that rig is the default `rig` behavior.
`docs/rig-guide.md` gives a short user-facing guide for using `rig`.

#23 is complete: v1 was removed, rig was promoted to `rig`, and the build emits
...
Runtime routing from legacy `rig` to rig was intentionally avoided. The approved
direction was to remove v1 and promote rig as the `rig` CLI in a deliberate
cutover commit.

### Follow-Up Progress

- #25 complete: `src/rig/runtime-executor.ts` defines the rig runtime
  ...
  validated rig config for repo-inferred and explicit `--config` command paths,
  ...
  `RigRuntimeExecutorLive` invokes in order. Lane config now carries
  ...
  workspace, installs the resulting executable into the rig-managed bin root,
  ...
  `rig init --project <name> --path <path>` now writes a valid lane-wired rig
  ...
  data paths under the isolated rig state root. A Pantry-like fake app flow now
  proves a routed web component, SQLite component, and installed CLI component
  with `installName: "pantry"` can deploy live and generated targets through rig
  ...
  `rig list` now reads global rig project/deployment inventory from
  `rigd.webReadModel`, with `--json` for structured tooling output.
  #27 is complete: the launchd
  process-supervisor provider installs
  rig-namespaced plists, bootstraps them with launchctl, removes them on down,
  ...
  rig-namespaced Caddyfile routes from deployment domains to localhost
...
`${dataRoot}`. It lives under isolated rig state rather than the app repository:
...
Caddy remains the first router provider for Rig. Traefik and Pangolin are
