...
export class NodeFileSystem implements FileSystemService {
  ...
  rename(src: string, dest: string): Effect.Effect<void, FileSystemError> {
    return Effect.tryPromise({
      try: async () => {
        await mkdir(dirname(dest), { recursive: true })
        await rename(src, dest)
      },
      catch: toError("rename", `${src} -> ${dest}`, "Ensure source exists and destination directory is writable."),
    })
  }
