...
import { RigDeployIntents, RigDeployIntentsLive } from "./deploy-intent.js"
...
const defaultHomeConfig: RigHomeConfig = {
  deploy: {
...
const runWithDeployIntents = async <A>(
  effect: Effect.Effect<A, unknown, RigDeployIntents>,
  options?: {
  ...
  const deployments = Layer.provide(
    RigDeploymentManagerLive,
    Layer.succeed(RigDeploymentStore, store),
  )
...
describe("GIVEN rig deploy intent model WHEN resolving pushes and CLI deploys THEN behavior is covered", () => {
  ...
  test("GIVEN dirty or stale release edge cases WHEN resolving THEN structured errors are returned", async () => {
    ...
    expect(result.dirty._tag).toBe("RigRuntimeError")
    expect(result.dirty.details?.reason).toBe("dirty")
    expect(result.stale._tag).toBe("RigRuntimeError")
