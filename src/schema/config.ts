...
export const BinServiceSchema = z
  .object({
    name: ServiceNameSchema,
    type: z.literal("bin").describe("bin = CLI tool installed to the rig-managed bin directory under ~/.rig/bin by default."),
    entrypoint: z
      .string()
      .min(1)
      .describe(
  ...
  .describe("CLI tool service. Built and/or installed to the rig-managed bin directory under ~/.rig/bin by default.")
