...
class MemoryDeploymentStore implements V2DeploymentStoreService {
  ...
  readonly ensured: V2DeploymentRecord[] = []
  readonly removed: V2DeploymentRecord[] = []
  ...
  ensureState(record: V2DeploymentRecord) {
    this.ensured.push(record)
    return Effect.void
  }

  removeState(record: V2DeploymentRecord) {
...
const runWithStore = async <A>(
  effect: Effect.Effect<A, unknown, V2DeploymentManager>,
) => {
...
const runWithFileStore = <A>(
  ...
  const layer = Layer.provide(V2DeploymentManagerLive, V2FileDeploymentStoreLive)
...
describe("GIVEN v2 generated deployments WHEN materializing inventory THEN behavior is covered", () => {
  ...
  test("GIVEN file-backed store WHEN materializing THEN inventory and generated state are persisted", async () => {
    const stateRoot = await mkdtemp(join(tmpdir(), "rig-v2-deployments-"))
    ...
    try {
      ...
      const inventory = JSON.parse(rawInventory) as V2DeploymentRecord[]
      expect(inventory.map((entry) => `${entry.kind}:${entry.name}`)).toEqual([
