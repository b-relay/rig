...
const runWithFileStore = <A>(effect: Effect.Effect<A, unknown, V2RigdStateStore>) =>
  Effect.runPromise(effect.pipe(Effect.provide(V2FileRigdStateStoreLive)))

describe("GIVEN rigd persistent state WHEN written under the v2 root THEN restart evidence is durable", () => {
  test("GIVEN runtime evidence WHEN persisted THEN it can be loaded by a fresh store", async () => {
    const stateRoot = await mkdtemp(join(tmpdir(), "rig-v2-rigd-state-"))
  ...
  test("GIVEN provider observations WHEN persisted THEN reconstruction separates confirmed and missing evidence", async () => {
    ...
    try {
      const reconstructed = await runWithFileStore(
        ...
        }).pipe(Effect.provide(V2ProviderRegistryLive("default"))),
      )
