    ...
    for (const scenario of [
      ...
      if (scenario === "rejected")
        response = Response.json(
          { error: { code: "REJECTED", message: "Deploy rejected" } },
          { status: 409 },
        );
      if (scenario === "accepted")
        response = Response.json({ result: { accepted: true } });
      if (scenario === "unreachable") await server.stop(true);
      let output = "";
      const exit = await runRemoteHelper("rig://localhost/example", {
        repoPath: directory,
        input: (async function* () {
          yield* ["push refs/heads/main:refs/heads/main", "", ""];
        })(),
        output: {
          write(value) {
            output += value;
      ...
      expect(output.includes("ok refs/heads/main")).toBe(
        scenario === "deployed",
      );
    }
  } finally {
