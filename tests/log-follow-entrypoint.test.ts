...
test("compiled follow uses production scheduling and terminates on SIGTERM without continued reads", async () => {
  ...
  const server = startControlPlane({
    port: 0,
    token: "isolated-follow-token",
    instanceId: "follow-fixture",
  ...
  try {
    ...
    expect({ code: buildCode, error: buildCode ? buildError : "" }).toEqual({
      code: 0,
      error: "",
    });
    ...
    const exit = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        watchdog = setTimeout(
          () => reject(new Error("Compiled follow failed to terminate")),
          1000,
        );
      }),
    ]);
  ...
  const server = startControlPlane({
    ...
    async handle(request) {
      ...
      await writeFile(
        join(target.logRoot, "target.jsonl"),
        JSON.stringify({
          timestamp: `2026-09-09T12:00:${String(requests.length).padStart(2, "0")}Z`,
          component: "app",
          stream: "stdout",
          line: `line ${requests.length}`,
        }) + "\n",
        { flag: "a" },
      );
      return {
