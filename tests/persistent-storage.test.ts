  ...
  for (const target of ["local", "live", "preview"] as const) {
    ...
    expect(first).toMatchObject({
      ...
      env: {
        DATA_DIR:
          target === "preview"
            ? "/root/targets/p/t/data/api/preview"
            : "/root/targets/p/t/data/api",
      },
    });
    expect({
      command: second.kind === "managed" && second.command,
      env: second.env,
    }).toEqual({
...
test("a config that declares a uses plugin is refused", () => {
  expect(() =>
    parseProjectConfig({
      name: "demo",
      components: { db: { uses: "sqlite", path: "data/app.db" } },
    }),
...
test("prepare links the checkout's .convex/local/default to the persistent Convex state so a new revision reuses it", async () => {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "rig-convex-state-")),
  );
  roots.push(root);
  const stateDir = join(root, "data", "convex", "api");
  const link = (workspace: string) =>
    join(workspace, ".convex", "local", "default");
  const first = join(root, "revisions", "r1"),
    second = join(root, "revisions", "r2");
...
test("prepare refuses to replace a real .convex/local/default directory in the checkout with a tagged error", async () => {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "rig-convex-conflict-")),
  );
  roots.push(root);
  const workspace = join(root, "revisions", "r1");
  await mkdir(join(workspace, ".convex", "local", "default"), {
    recursive: true,
  });
  await expect(
    effects(root).prepare(
      record(root, workspace, join(root, "data", "convex", "api")),
    ),
...
test("prepare leaves a local Target's in-workspace Convex state alone", async () => {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "rig-convex-local-")),
  );
  roots.push(root);
