...
const notReady = (reason = "not yet"): HealthCheck => ({
  ready: false,
  reason,
});
...
function fixture(
  ...
  const record: TargetRecord = {
    ...
    plan: {
      ...
      preparedComponents: [],
      components: ["prior", "new"].map((name) => ({
        name,
        kind: "managed",
        command: "serve",
        ...(options.healthChecks === false ? {} : { port: 4000 }),
        readyTimeout: 1,
        env: {},
        dependsOn: options.dependsOn?.[name] ?? [],
...
test("a process that has exited is reported with its exit code before the first health poll instead of after readyTimeout", async () => {
  ...
  const f = fixture(
    ...
    { crashes: { "readiness:new": 127 } },
  );
  await expect(f.lifecycle.up(f.record)).rejects.toMatchObject({
    code: "PROCESS_EXITED",
    message:
      "new exited with code 127 before it became ready: the shell found no executable for its command.",
...
test("a component with no health check counts as started only after surviving the start grace period", async () => {
  ...
  let settled = false;
  const result = f.lifecycle.up(f.record).then((outcome) => {
    settled = true;
    return outcome;
  });
  await flush();
...
test("a component with no health check that exits during the start grace period is a failed start", async () => {
  const f = fixture(async () => ready, {
    healthChecks: false,
    crashes: { "readiness:new": 99 },
  });
  let failure: unknown;
  const result = f.lifecycle.up(f.record).catch((error) => {
    failure = error;
  });
  await flush();
...
test("controlled deadline bounds uncooperative health and prevents late revival", async () => {
  ...
  const f = fixture((_component, _target, abort) => {
    signal = abort;
...
test("a supervisor that never answers the liveness observation is bounded by the same deadline", async () => {
  ...
  f.stalled.observation = new Promise((resolve) => {
    answer = resolve;
  });
  let failure: unknown;
  const result = f.lifecycle.up(f.record).catch((error) => {
...
test.each([0.05, 0.1, 0.25])(
  "unhealthy readiness expires at %ss even during a retry delay",
  async (timeout) => {
    let calls = 0;
    const f = fixture(async () => {
      calls++;
...
test("foreign checkpoint identity fails before any Target effects or scheduling", async () => {
  const f = fixture(async () => ready);
  await expect(
    f.lifecycle.up(f.record, { ...f.checkpoint, targetId: "other" }),
  ).rejects.toMatchObject({ code: "EFFECTS_SCOPE" });
  expect(f.events).toEqual([]);
  expect(f.timing.pending).toBe(0);
});
...
test("an already running dependency must pass its health check before a dependent starts", async () => {
  const f = fixture(
    async (component) =>
      component.name === "prior"
        ? { ready: false, reason: "HTTP 500" }
        : { ready: true },
    { dependsOn: { new: ["prior"] } },
  );
  let failure: unknown;
  ...
  expect(failure).toMatchObject({
    code: "HEALTH_FAILED",
    details: { component: "prior", lastCheck: "HTTP 500" },
  });
  expect(f.events).not.toContain("start:readiness:new");
  expect([...f.running]).toEqual(["readiness:prior"]);
  const healthy = fixture(async () => ({ ready: true }), {
    dependsOn: { new: ["prior"] },
  });
  expect(await healthy.lifecycle.up(healthy.record)).toEqual({
    outcome: "started",
  });
