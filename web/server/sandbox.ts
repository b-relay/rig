...
/** Runs `rigd <command>` against one Rig root and answers with its exit code and what it printed. */
export type RunRigd = (
...
export function sandboxDaemon(root: string, run: RunRigd) {
  return {
    ...
    async start(): Promise<void> {
      const { exitCode, output } = await run("install", root);
      if (exitCode !== 0)
        throw new RigError(
          "WEB_SANDBOX_FAILED",
    ...
    async stop(): Promise<boolean> {
      return (await run("uninstall", root)).exitCode === 0;
    },
  };
}
